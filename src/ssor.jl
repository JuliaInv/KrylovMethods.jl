...
function ssorPrecTrans!(A::SparseMatrixCSC,x::Vector,b::Vector,OmInvD::Vector=1./diag(A))
# ssorPrecTrans!(A::SparseMatrixCSC,x::Vector,r::Vector,OmInvD::Vector=1./diag(A))
#
# Applies one SSOR step to the linear system A*x = b, for symmetric A. For efficiency, it works on the
...
#	OmInvD  - weighted diagional, that is, omega./diag(A)
#
# Output: This method changes x and on the fly. The vector b is unchanged.
#
# This version is well-suited as a preconditioner, for example, for CG.

##  Example for solving Ax=b, when A is Sparse Symmtric Positive definite:
# omega = 1.2;
# d = omega./diag(A);
...
for j=n:-1:1
	r = b[j]
	for gidx = A.colptr[j] : A.colptr[j+1]-1
		r -= A.nzval[gidx]*x[A.rowval[gidx]]
	end
	r *= OmInvD[j]
	x[j] += r
end
